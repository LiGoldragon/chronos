...
//! Datomic (the daemon's reply-rendering path) and rkyv (the
//! UDS frame path). The CLI's print loop renders responses
//! as canonical Datomic, so any breakage here breaks `chronos 'GetTime'`.

use chronos::{
    EclipticLongitude, EpochTaiNanos, ErrorMessage, Latitude, Location, LocationSource, Longitude, Response,
    SolarEvent, SolarEventKind, ZodiacalTime,
};

fn round_trip_datomic(response: &Response) -> Response {
    let text = response.to_text();
    let again = Response::from_text(&text).expect("Datomic embody");
    assert_eq!(*response, again, "Datomic round-trip changed value via text {text:?}");
    again
}
...
#[test]
fn error_message() {
    let response = Response::Error { message: ErrorMessage::try_new("ephemeris file not found".into()).unwrap() };
    round_trip_datomic(&response);
    round_trip_rkyv(&response);
}
