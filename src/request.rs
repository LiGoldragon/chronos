    ...
    fn incorporate(site: datom_codec::Site<'_>) -> core::result::Result<Self, datom_codec::Fault> {
        ...
        match variant.name {
            ...
            "SetLocation" => {
                ...
                Ok(Self::SetLocation { latitude: p.position()?, longitude: p.position()? })
            }
            "Subscribe" => {
                let mut p = Headed::positions(variant, 1)?;
                Ok(Self::Subscribe { kinds: p.position()? })
            }
            name => Err(variant
...
impl Conceivable<Datom> for Request {
    ...
    fn conceive(&self) -> core::result::Result<Situated<Datom>, Self::Fault> {
        let body = |value: &dyn Conceivable<Datom, Fault = core::convert::Infallible>| {
            value.conceive().expect("infallible Datom ascent").1
        };
        let datom = match self {
            ...
            Self::SetLocation { latitude, longitude } => Datom::Variant(
                Symbol::try_from("SetLocation").expect("static symbol"),
                Box::new(Datom::Struct(vec![body(latitude), body(longitude)])),
            ),
            Self::Subscribe { kinds } => Datom::Variant(
                Symbol::try_from("Subscribe").expect("static symbol"),
                Box::new(Datom::Struct(vec![body(kinds)])),
            ),
        };
